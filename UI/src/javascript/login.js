// ======================================
// login.js – Connect Bootstrap form to FastAPI JWT backend (user.py)
// Endpoints:
//   POST /api/auth/login   -> { access_token, access_expires_at, refresh_token, refresh_expires_at }
//   POST /api/auth/refresh -> { ... }
//   GET  /api/me           -> user profile (Bearer access)
//   POST /api/auth/logout  -> revoke current access (Bearer access)
// ======================================
...
(async () => {
  ...
  // Backend user.py mặc định chạy 8000:
  ...
  function ensureAlertHost() {
    ...
    if (!host) {
      ...
      if (btnRow && btnRow.parentNode) {
        btnRow.parentNode.insertBefore(host, btnRow); // insert before submit row
      } else if (form) {
  ...
  async function ensureAccessToken() {
    ...
    if (isRefreshExpired()) throw new Error("Session expired. Please log in again.");
  ...
  const form = $("#loginForm");
  const btn = $("#loginBtn");
  const usernameEl = $("#username"); // holds email in this backend
  ...
  async function handleSubmit(e) {
    ...
    // Backend login expects: { identifier, password }
    ...
    try {
      ...
      localStorage.setItem(LS_KEYS.me, JSON.stringify(me || {}));

      // Redirect (tùy chỉnh trang đích của bạn)
      ...
      if (/Invalid credentials/i.test(msg)) {
        setFieldInvalid(usernameEl, true, "Invalid email.");
  ...
  // Auto-try session restore (if RT valid)
  try {
